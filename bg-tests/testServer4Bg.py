...
class MainClass:
    ...
    def handleServerhello(self, T):
        ...
        b = random.getrandbits(
            (8 * 16) - 2
        )  # Denne funktion angiver nogle tilfældige bits. Derfor (8 * 16)-2, da det tilfældige tal skal være 2 lavere.
        # print(f'b: {b}')
        B = pow(g, b, p)
        self.K = pow(A, b, p)
        # print(f'B: {B}')
    ...
    def handleData(self, T):
        ...
        data = (
            T.to_bytes(1) + L_bytes + firstline_bytes
        )  # Skal bruges til at udregne FCS.
        ...
        serverstring = firstline[
            12:-2
        ]  # Laver en string, hvor jeg slicer fra 12 og fjerne de sidste to elementer
    ...
    def sendClienthello(self, B):
        ...
        FCS = self.calculateFCS(
            data
        )  # Denne metode returner en integer. Skal laves til et byte-array, for at kunne transmitteres
    ...
    def encryptDecrypt(self, data_payload):
        if (
            not hasattr(self, "X") or self.X is None
        ):  # Hvis ikke klassen har self.X eller hvis self.X er None
            print(f"Initializing X the first time...")
            self.X = self.K.to_bytes(16)[
                13:16
            ]  # Skal kun initlialiseres 1 gang !!!!!!!!
        ...
        for i in range(len(data_payload)):
            ...
            keybyte = self.X.to_bytes(3)[
                1
            ]  # Laver self.X til 3 bytes, og udvælger den midterste
