...
startMessage = fileReader.readline().strip()     # Læser den første linje og fjerner newline fra slutningen af strengen 
print(startMessage)
...
while gameActive:
    ...
    linje1 = fileReader.readline()               # Definerer den første linje som vi modtager fra serveren.
    ...
    if "ILLEGAL" in linje1:                      # Undersøger om ILLEGAL er i serverbeskeden
        printBoardState(newestBoard, playerIs)   # Newestboard er den nyese board state som serveren returnerer. Den Defineres på linje 129.
        print(linje1.strip())
        
    # Tager det nye board serveren giver, og sætter det op på en visuel format    
    elif "BOARD IS" in linje1:
        newestBoard = linje1.strip()[-9:]        # Er spillets board state, hvor strip() fjerne eventuelle \n, og giver os de 9 sidste elementer i vores string.
